...
class SqueezeAndExcite(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.pool(x)
        # ----------------------------------------------------------------------------#
        #   view函数，可以将tensor变成一维数组的顺序，按照括号内大小重新填充
        #   如果是torch.view(-1)，则原张量会变成一维的结构
        #   如果是torch.view(参数a，-1)，则表示在参数b未知，参数a已知的情况下自动补齐列向量长度
        # ----------------------------------------------------------------------------#
        out = out.view(b, -1)
...
def mobilenetv3(pretrained=False):
    model = MobileNetV3(type='large', num_classes=1000)
    return model
